...
const withWaveHeader = (data, numberOfChannels, sampleRate) => {
  ...
  // const headerData = new DataView(header);

  // let position = 0;

  // const chunkSize = data.byteLength / 2 + 44;

  /* const writeString = (str) => {
    for (let i = 0; i < str.length; i++) {
      headerData.setUint8(position + i, str.charCodeAt(i));
    }
    position += str.length;
  } */

  /* const writeUint32 = (value) => {
    headerData.setUint32(position, value, true);
    position += 4;
  } */

  /* const writeUint16 = (value) => {
    headerData.setUint16(position, value, true);
    position += 2;
  } */
